...
export default function NewScreen() {
    ...
    }, [amountStr, categoryId, user, submitting, profile, type, notes, router, accounts]);
            ...
            <SafeAreaView style={{ flex: 1 }} edges={["top"]}>
                ...
                >
                    ...
                    <View className="mb-3 px-5">
                        <View className="bg-muted rounded-[18px] px-4 py-3.5 flex-row items-center gap-3">
                            <StickyNote size={20} color="#9ca3af" />
                            <TextInput
                                value={notes}
                                ...
                                maxLength={100}
                                returnKeyType="done"
                                className="flex-1 text-base text-foreground font-medium p-0"
                            />
                            {notes.length > 0 && (
                                <TouchableOpacity onPress={() => setNotes("")} activeOpacity={0.6}>
                                    <Icons.XCircle size={18} color="#d1d5db" />
                                </TouchableOpacity>
                            )}
                        </View>
                    </View>
                    ...
                    <View className="mb-4 px-5">
                        ...
                        >
                            <Icons.CalendarDays size={14} color="#6B7280" strokeWidth={2} />
                            <Text style={{ fontSize: 13, fontWeight: "600", color: "#374151" }}>
                                {(() => {
                                    ...
                                    if (isToday) return `Today · ${selectedDate.toLocaleDateString("en-US", { month: "short", day: "numeric" })}`;
                                    if (isYesterday) return `Yesterday · ${selectedDate.toLocaleDateString("en-US", { month: "short", day: "numeric" })}`;
                                    return selectedDate.toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric", year: "numeric" });
                                })()}
                            </Text>
                            <Icons.ChevronDown size={13} color="#9ca3af" strokeWidth={2.5} />
                        </TouchableOpacity>
